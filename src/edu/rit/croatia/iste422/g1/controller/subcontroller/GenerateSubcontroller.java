...
/**
 * Handles the "Generate MySQL" action within the application.
 * <p>
 * The {@code GenerateSubontroller} is responsible for triggering the script generation, trigger of parsing it, and updating the UI and model with the script data.
 ...
 * @see SchemaView
 * 
 * @author Mariia Zarovska, Marko Skific
 * @version 2.8
 */
public class GenerateSubcontroller extends Subcontroller {


    /**
     * Logger instance for recording application events and debugging information.
     */
    public static final Logger logger = LogManager.getLogger(GenerateSubcontroller.class);


    /**
     * Constructs an {@code GenerateSubcontroller} and initializes it with the given view and model.
     *
     ...
     */
    public GenerateSubcontroller(SchemaView schemaView, SchemaModel schemaModel){
        super(schemaView, schemaModel);
        logger.info("Set GenerateSubcontroller successfully...");
    }


    /**
     * Handles the "Generate MySQL" action event.
     * <p>
     * This method triggers the schemaModel to generate the "mysql" type of script and then triggers the 
     * schemaView to interpret, set the script and refresh the view to display it
     * </p>
     *
     * @param event the action event triggered by the "Generate MySQL" action.
     */
    @Override
    public void handle(ActionEvent event) {
        logger.info("Attempting to retrieve script file...");
        schemaModel.generateScript("mysql");
        schemaView.setOutputView(schemaModel.getScript());
        logger.info("Retrieved Generated script file successfully");
    }
